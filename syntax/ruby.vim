...
if !exists("b:ruby_no_expensive") && !exists("ruby_no_expensive")
  ...
  syn region rubyCurlyBlock   start="{" end="}"							  contains=ALLBUT,@rubyNotTop
  ...
  syn region rubyConditionalExpression matchgroup=rubyConditional start="\%(\%(^\|\.\.\.\=\|[{:,;([<>~\*/%&^|+=-]\|\%(\<[_[:lower:]][_[:alnum:]]*\)\@<![?!]\)\s*\)\@<=\%(if\|unless\)\>" end="\<end\>" contains=ALLBUT,@rubyNotTop
